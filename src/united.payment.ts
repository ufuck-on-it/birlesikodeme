...
import {
  ...
  BinListResponse,
  Card,
  CardCheckPointsRequest,
  CardCheckPointsResponse,
  CardListResponse,
  CommissionInquiryRequest,
  CommissionInquiryResponse,
  CommissionInstallResponse,
  CommissionResponse,
  CustomerCard,
  CustomerCardsResponse,
  DeleteCustomerCardRequest,
  DeleteCustomerCardResponse,
  InstallmentInquiryResponse,
  IptalIadeServisiRequest,
  IptalIadeServisiResponse,
  MerchantData,
  ...
  SaveCardWithoutTransactionRequest,
  SaveCardWithoutTransactionResponse,
  TransactionData,
...
export class UnitedPayment {
  ...
  public async authorizePaymentIFrame(request: PaymentAuthorizationRequest): Promise<PaymentAuthorizationResponse> {
    try {
      // HASH hesaplama
      const calculatedHash = this
        .calculateHash(`${this.hashPassword}+${request.userCode}+${request.rnd}+${request.txnType}+${request.totalAmount}+
  ...
  public async noneSecurePayment(request: PaymentAuthorizationRequest): Promise<PaymentAuthorizationResponse> {
    try {
      ...
      const response = await this.httpClient.post<PaymentAuthorizationResponse>(
        '/api/ppg/Payment/NoneSecurePayment',
        request
      );
      const data = response.data;
  ...
  public async paymentInquiry(request: PaymentInquiryRequest): Promise<PaymentInquiryResponse> {
    try {
      // HASH hesaplama
      request.hash = this.calculateHash(
        `${this.hashPassword}+${request.rnd}+${request.orderNo}+${request.totalAmount}`
      );
  ...
  async saveCardWithoutTransaction(
    request: SaveCardWithoutTransactionRequest
  ): Promise<SaveCardWithoutTransactionResponse> {
    try {
      const response = await this.httpClient.post<SaveCardWithoutTransactionResponse>(
        '/api/ppg/Payment/SaveCardWithoutTransaction',
        request
      );
      const data = response.data;
  ...
  public async getTransactionList(
    ...
    try {
      ...
      const response = await this.httpClient.post<TransactionListResponse>(
        '/api/ppg/Payment/GetTransactionList',
        request
      );
      const data = response.data;
  ...
  public async commissionInquiry(request: CommissionInquiryRequest): Promise<CommissionInquiryResponse> {
    try {
      const response = await this.httpClient.post<CommissionInquiryResponse>(
        '/api/ppg/Payment/CommissionInquiry',
        request
      );
      const data = response.data;
  ...
  public async installmentInquiry(
    merchantId: number,
    bin: string,
    txnType: string
  ): Promise<InstallmentInquiryResponse> {
    try {
      const response = await this.httpClient.get<InstallmentInquiryResponse>(
        `/api/ppg/Payment/HasInstallment/${merchantId}/${bin}/${txnType}`
      );
      const data = response.data;
  ...
  public async getCustomerCards(memberId: number, merchantId: number, customerId: string): Promise<CustomerCard[]> {
    try {
      const response = await this.httpClient.get<CustomerCardsResponse>(
        `/api/ppg/Payment/GetCustomerCards/${memberId}/${merchantId}/${customerId}`
      );
      const data = response.data;
  ...
  public async deleteCustomerCard(request: DeleteCustomerCardRequest): Promise<DeleteCustomerCardResponse> {
    try {
      const response = await this.httpClient.put<DeleteCustomerCardResponse>(
        '/api/ppg/Payment/DeleteCustomerCard',
        request
      );
      const data = response.data;
